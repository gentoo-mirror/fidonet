...
SRC_URI="http://www.catb.org/~esr/terminfo/termtypes.tc.gz mirror://gentoo/${MY_P}.tar.bz2 mirror://gentoo/${MY_P}-patches-${PATCHVER}.tar.bz2"
...
src_compile() {
	TARGETS=libtermcap.so.${PV}
	use static-libs && TARGETS="${TARGETS} libtermcap.a"
	emake ${TARGETS} || die
}
src_install() {
	use static-libs && dolib libtermcap.a
